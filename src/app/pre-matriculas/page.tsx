...
import { ArrowLeft, UserPlus, Check, ArrowUpDown, FileDown, Phone, MessageCircle, Pencil, Search, Plus, Trash2 } from 'lucide-react';
...
import { PreMatriculaData, addPreMatricula, updatePreMatriculaData, getPreMatriculas, deletePreMatricula } from './data';
...
const StatusBadge = ({ status }: { status: PreMatriculaData['status'] }) => {
    ...
    const variant = status === 'pre' || status === 'aguardando_pagamento' ? 'outline' : 'default';

    return <Badge variant={variant} className={statusClasses[status]}>{statusText[status]}</Badge>;
};
...
export default function PreMatriculasPage() {
  ...
  const [editingMatricula, setEditingMatricula] = useState<PreMatriculaData | null>(null);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [deletingMatricula, setDeletingMatricula] = useState<PreMatriculaData | null>(null);


  const [sortConfig, setSortConfig] = useState<{ key: SortKey; direction: 'ascending' | 'descending' }>({ key: 'criado', direction: 'descending' });
  ...
  const handleCreateMatricula = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCity) {
        toast({ title: 'Erro', description: 'Selecione uma cidade primeiro.', variant: 'destructive' });
        return;
    }

    if (!validateCpf(cpf)) {
        toast({ title: 'Erro', description: 'Por favor, insira um CPF válido.', variant: 'destructive' });
        return;
    }
  
    if (matriculas.some(m => m.cpf.replace(/[^\d]/g, '') === cpf.replace(/[^\d]/g, ''))) {
        toast({ title: 'Erro', description: 'Este CPF já está cadastrado nesta cidade.', variant: 'destructive' });
        return;
    }
    ...
    const newMatricula: PreMatriculaData = {
      ...
      status: 'pre',
      local: selectedCity,
      criado: new Date().toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(',', ''),
    };
    ...
    toast({ title: 'Sucesso!', description: 'Pré-matrícula criada com sucesso.' });
  }

  const sortedMatriculas = useMemo(() => {
    let sortableItems = [...matriculas];
    if (sortConfig.key) {
  ...
  const handleConfirmDelete = () => {
    ...
    toast({ title: "Sucesso!", description: `Pré-matrícula de "${deletingMatricula.nome}" excluída.` });
  ...
  const handleUpdateMatricula = () => {
    ...
    if (!validateCpf(editingMatricula.cpf)) {
      toast({ title: 'Erro', description: 'Por favor, insira um CPF válido para atualizar.', variant: 'destructive' });
      return;
    }
  
    if (matriculas.some(m => m.id !== editingMatricula.id && m.cpf.replace(/[^\d]/g, '') === editingMatricula.cpf.replace(/[^\d]/g, ''))) {
        toast({ title: 'Erro', description: 'Este CPF já está cadastrado em outra pré-matrícula.', variant: 'destructive' });
        return;
    }
    ...
    toast({ title: 'Sucesso!', description: 'Pré-matrícula atualizada com sucesso.' });
  };
  ...
  const handleExportCSV = () => {
    const headers = ['#ID', 'Nome', 'Status', 'Local', 'CPF', 'Whatsapp', 'Criado'];
    const rows = sortedMatriculas.map(m => [m.id, m.nome, m.status, m.local, m.cpf, m.whatsapp, m.criado].join(','));
  ...
  const handleExportExcel = () => {
    ...
    downloadFile(excelContent, 'pre-matriculas.xls', 'application/vnd.ms-excel');
  };
  ...
  const handleExportPDF = () => {
    ...
    (doc as any).autoTable({
        ...
        body: sortedMatriculas.map(m => [m.id, m.nome, m.status, m.local, m.cpf, m.whatsapp, m.criado]),
    });
  ...
  const endRecord = Math.min(currentPage * itemsPerPage, sortedMatriculas.length);
      ...
      <div className="flex min-h-screen flex-col bg-background">
        ...
        <main className="flex-1 p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className='lg:col-span-1'>
                <Card>
                    <CardHeader>
                        <CardTitle className='flex items-center gap-2'><Plus size={24} /> Criar Pré-Matrícula</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleCreateMatricula} className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="cpf" value={cpf} onChange={handleCpfChange} placeholder="000.000.000-00" required />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="whatsapp">* Whatsapp:</Label>
                                <Input id="whatsapp" value={whatsapp} onChange={handleWhatsappChange} placeholder="(00) 00000-0000" required />
                            </div>
                            <div className="flex items-center space-x-2 pt-2">
                                <Switch id="sendWhatsapp" checked={sendWhatsapp} onCheckedChange={setSendWhatsapp} />
                                <Label htmlFor="sendWhatsapp">Enviar confirmação de pré matrícula via whatsapp?</Label>
                            </div>
                            <div className='pt-4'>
                ...
                <Card>
                    <CardHeader>
                        <div className="flex flex-wrap items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                <Input id="search" placeholder="Pesquisa..." className="h-9"/>
                            </div>
                            <div className="flex flex-wrap items-center gap-2">
                                <span className="text-sm text-muted-foreground">Mostrando</span>
                                <Select defaultValue="25" onValueChange={handleItemsPerPageChange}>
                    ...
                    <CardContent>
                        <div className="overflow-x-auto rounded-md border">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead>
                                            <Button variant="ghost" size="sm" onClick={() => requestSort('local')}>Local<ArrowUpDown className="ml-2 h-4 w-4" /></Button>
                                        </TableHead>
                                        <TableHead>
                                ...
                                {paginatedMatriculas.length === 0 ? (
                                    ...
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>
                                            ...
                                            {userRole === 'mestre' && (
                                                <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleDeleteClick(item)}>
                         ...
                         <div className="flex items-center justify-between pt-4">
                             <span className="text-sm text-muted-foreground">Mostrando {startRecord} a {endRecord} de {sortedMatriculas.length} registros</span>
                             <div className="flex gap-2">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Faça alterações nos dados da pré-matrícula aqui. Clique em salvar quando terminar.
            </DialogDescription>
          </DialogHeader>
          {editingMatricula && (
       ...
       <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <AlertDialogContent>
              ...
              <AlertDialogDescription>
                  Esta ação não pode ser desfeita. Isso irá excluir permanentemente a pré-matrícula de
