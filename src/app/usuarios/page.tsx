...
export default function UsuariosPage() {
    ...
    if (!authStatus) {
      ...
      if (roleFromSession === "pessoa") {
        ...
        const user = allUsers.find(
          (u) => u.name === userName && u.role === "pessoa"
        );
        if (user && user.city) {
  ...
  useEffect(() => {
    if (isClient) {
      ...
      return () =>
        window.removeEventListener("usersDataChanged", handleUsersChanged);
    }
  }, [selectedCity, isClient]);
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    if (!userCity) {
      toast({
        title: "Erro",
        description: "Por favor, selecione uma cidade para o usuário.",
        variant: "destructive",
      });
      return;
    }
    if (!email.endsWith("@gcscursos.com.br")) {
      toast({
        ...
        description: "O e-mail deve terminar com @gcscursos.com.br",
        variant: "destructive",
      });
      return;
    }
    ...
    if (cityUsers.some((u) => u.email === email)) {
      toast({
        title: "Erro",
        description: "Este e-mail já está em uso nesta cidade.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      title: "Sucesso!",
      description: "Usuário criado localmente com sucesso.",
    });
    setName("");
    setEmail("");
    setPassword("");
  ...
  const handleUpdateUser = async () => {
    ...
    if (
      ...
      toast({
        title: "Erro",
        description:
          "O e-mail informado já está em uso por outro usuário nesta cidade.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Sucesso!",
        description: `Usuário "${deletingUser.name}" excluído.`,
      });
    } catch (error: any) {
      ...
      if (error.code === "auth/requires-recent-login") {
        description =
          "Esta é uma operação sensível. Por favor, faça login novamente para excluir o usuário.";
      }
                  ...
                  <form onSubmit={handleAddUser} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id="email"
                        type="email"
                        placeholder="email@gcscursos.com.br"
                        value={email}
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        onValueChange={(v: UserData["role"]) => setRole(v)}
                        value={role}
                      >
                        <SelectTrigger>
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Lista de todos os usuários com acesso ao sistema na cidade de{" "}
                  <span className="font-semibold">
                    {selectedCity || "Nenhuma"}
                  </span>
                  .
                </CardDescription>
              </CardHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Altere os dados do usuário. Clique em salvar para aplicar as
              mudanças.
            </DialogDescription>
          </DialogHeader>
          {editingUser && (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              o usuário
              <span className="font-bold"> {deletingUser?.name}</span> do
              sistema.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
